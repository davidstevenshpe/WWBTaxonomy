...
<syswf:control mode="script" action="update" caption="${prefix}_update" affects="../.." loadingMask="true"/>
<syswf:input id="${prefix}_updated_id" name="updatedId" value="${column.id}" mode="hidden"/>
...
<%--
<syswf:input value="${column}" property="label" name="name" hint="${column.label}">
    <syswf:attribute name="class">UI Form ${editableLabelClass}</syswf:attribute>
</syswf:input>
<input type="text" value="${column.label}" name="name" title="${column.label}" class="UI Form ${editableLabelClass}" readonly="readonly">
--%>

<c:if test="${column.editableLabel}">
    <label id="${prefix}_label" class="UI Form Label Inline EditableLabelStatic" ><c:out value="${column.label}" /></label>
    <input id="${prefix}_input" type="text" name="name" value="<c:out value="${column.label}" />" class="UI Form EditableLabel x-hidden" readonly="readonly"/>
</c:if>
<c:if test="${not column.editableLabel}">
    <span class="UI Form EditableLabelDisabled"><c:out value="${column.label}" /></span>
</c:if>
