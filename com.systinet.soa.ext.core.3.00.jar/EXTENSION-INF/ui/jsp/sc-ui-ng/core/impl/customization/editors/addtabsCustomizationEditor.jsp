...
<div id="${prefix}content">
    <div class="UI Align x-tip-body">
    	${tabName}:
        <div>
          <syswf:input value="${valueHolder}" property="value" name="text" id="${prefix}inputValue" /> <span>&nbsp;</span> <syswf:control mode="button" caption="${buttonCaption}" action="add" icon="plus-circle"/>
        </div>
        ${tabHint}:
        <div>
        <syswf:input value="${valueHolder}" property="hint" name="hint" id="${prefix}inputHint" />	
        </div>	
		<div class="clear"></div>
    </div>
</div>
...
<c:if test="${empty popupWidth}">
    <c:set var="popupWidth" value="300" />
</c:if>
<c:if test="${empty inputWidth}">
    <c:set var="inputWidth" value="${popupWidth - 110}" />
</c:if>
...
var values = [
  ...
  var combos = {
      ...
      cbsite.on('select', function() {
          var element = Ext.get('${prefix}inputValue');
...
Ext.onReady(function() {
    ...
    var tip = new Ext.ToolTip({
        ...
        //html: '<syswf:addSlashes>${tipContent}</syswf:addSlashes>',
            listeners: {
            beforeshow: function() {
                Ext.get('${prefix}inputValue').focus();
            }
        }  
    });
